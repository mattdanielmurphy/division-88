...
module.exports = async (req, res) => {
	...
	const allModelsUpdated = models.map(
		async ([ Model, AdminModel ]) => await updateAllSubdocuments(Model, AdminModel, Model.modelName)
	)

	// set changesMade to false
	const updateAdminInfo = await AdminInfo.updateOne({}, { changesMade: false }).then(
