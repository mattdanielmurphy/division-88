...
export default class extends React.Component {
	...
	updateBlocks = (blocks) => {
		...
		post.blocks = blocks
		this.props.AdminAPI.post('/posts', post).then((res) => console.log(res)).catch((err) => console.log(err))
		this.props.setChangesMade(true)
	}
	getPost = async (postTitle) => {
		const post = await this.props.AdminAPI.get(`/posts/${postTitle}`).then((r) => r.data)
	...
	handleTitleChange = (e) => {
		...
		console.log('changing title...', post)
		this.props.AdminAPI
			.post(`/posts`, post)
			.then((res) => {
				console.log('res', res)
