...
export default class CellEditor extends React.Component {
	state = {}
	getCell = async (index) => {
		return await this.props.AdminAPI.get(`/grids/index/cells/${index}`).then((r) => r.data)
	}
	updateCell = async (index, cell) => {
		const result = await this.props.AdminAPI.post(`/grids/index/cells/${index}`, cell).then((r) => {
			return r.data
		})
		return result
	}
	...
	handleSubmit = async (e) => {
		...
		const result = await this.props.AdminAPI
			.post(`/grids/index/cells/${this.state.index}`, this.state.cell)
			.then((r) => {
				console.log(r)
				return r.data
			})
	...
	getCellsFromDatabase = async () => {
		return await this.props.AdminAPI.get('/grids/index/cells').then((r) => r.data)
	}
	updateState = (cell) => {
		cell = Object.assign(this.state.cell, cell)
		this.setState({ cell }, () => this.handleSubmit())
	}
	...
	newCell = async () => {
		console.log('creating new cell. state of layouts before anything happens:', this.state.layouts)
		const result = await this.props.AdminAPI.get('/grids/index/layouts')
		console.log('creating new cell. db layouts before anything happens:', result)
		...
		if (this.validateCell()) {
			...
			this.props.AdminAPI.post('/grids/index/layouts/newCell', obj).then((r) => {
	...
	deleteCell() {
		...
		this.props.AdminAPI.post(`/grids/index/cells/${this.state.index}/delete`).then((r) => {
			const { layouts, cells } = r.data
			this.props.updateGrid({ layouts, cells })
			...
			<div id="property-editor">
				...
				<form onSubmit={(e) => this.handleSubmit(e)} noValidate>
					...
					{this.state.cell.video ? (
						<VideoPicker updateState={(state) => this.updateState(state)} {...this.state.cell} />
					) : (
						<NonVideoOptions
