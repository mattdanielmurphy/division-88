...
class VideoBlockContent extends React.Component {
  state = {
...
class PhotoBlockContent extends React.Component {
  state = {
    src: this.props.src || '',
    ...
    <div className='content-block-image'>
      <div className='property-input'>
        ...
        <ImageUploader
          ...
          setImage={(url) =>
            this.handleInputChange({ path: 'src', value: url })
          }
        />
      </div>
      <div className='property-input'>
        <label>
          alt text (short description of image for accessibility purposes):
        </label>
        <input
          id='altText'
          onChange={(e) => this.handleInputChange({ e })}
...
class SoundCloudBlockContent extends React.Component {
  state = {
    src: this.props.src || '',
  }
  handleInputChange = async ({ e, path, value }) => {
...
class BlockChooser extends React.Component {
  render = () => (
    <div>
      ...
      <button onClick={() => this.props.chooseType('soundcloud')}>
        Soundcloud
      </button>
    </div>
  )
    ...
    <div className='content-block'>
      <div className='block-options'>
        <button
          ...
          onClick={() => this.props.addBlockAbove()}
        >
          Add block above
        </button>
        <button
          className='move-block-button'
          onClick={() => this.props.moveBlockUp()}
        >
          Move up
        </button>
        <button
          className='move-block-button'
          onClick={() => this.props.moveBlockDown()}
        >
          Move down
        </button>
      ...
      <button
        className='add-block-button'
        onClick={() => this.props.addBlockBelow()}
      >
        Add block below
      </button>
    </div>
...
export default class TextEditor extends React.Component {
  ...
  spliceBlock(index, numBlocksToDelete, newBlock) {
    ...
    if (newBlock)
      blocks = [
        ...
        newBlock,
        ...blocks.slice(index + numBlocksToDelete),
      ]
    else
      blocks = [
        ...blocks.slice(0, index),
  ...
  deleteBlock = (index) => {
    const deletingLastBlock =
      this.state.blocks.reduce(
        (count, block) => (block ? count + 1 : count),
        0,
      ) === 1
          ...
          <Droppable droppableId='droppable'>
            {(provided, snapshot) => (
              ...
              >
                {this.state.noBlocks ? (
                  <div>
                    <button onClick={() => this.addFirstBlock()}>
                      Add block
                    </button>
                  </div>
                ) : (
