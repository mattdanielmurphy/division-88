...
export default class extends React.Component {
	...
	publish = async () => {
		...
		if (result.data && result.data.updated) {
			window.alert('Successfully published. You may need to reload the browser, but probably not.')
			this.props.setChangesMade(false)
		} else if (result.data && !result.data.updated) {
			window.alert(`*********
PUBLISH FAILED!
*********
...
No error code was provided. The server might be down... try again in a few minutes.`)
		}
	}
	...
	revert = async () => {
		...
		if (result.data.updated) {
			window.alert('Successfully reset. You may need to reload the browser, but probably not.')
			this.props.setChangesMade(false)
		} else window.alert('Reset failed. The server might be down... try again in a few minutes.')
	}
